...
import { AlertCircle, Plus, ChevronDown, ChevronRight, Hash, Users, Loader2, Menu } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="h-8 gap-1">
                    ...
                    <span className="text-xs">Sub-groups</span>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel className="flex items-center justify-between">
                    ...
                    {isManagerOrAbove(user) && (
                      <Button
                        ...
                        variant="ghost"
                        className="w-6 h-6"
                        onClick={(e) => { e.preventDefault(); setShowCreateSubGroup(true); }}
                      >
                  ...
                  <DropdownMenuSeparator />
                  {subgroups.length === 0 ? (
                    <div className="p-4 text-center text-xs text-muted-foreground">No sub-groups</div>
                  ) : (
                    subgroups.map(sg => (
                      <DropdownMenuItem key={sg._id} asChild>
                        <Link href={`/groups/${groupId}/sub/${sg._id}`} className="w-full cursor-pointer">
